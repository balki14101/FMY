...
export default function Saturday(props) {
  ...
  const [breakEndHourSun, setBreakEndHourSun] = useState("0")
	...
	function getValueFromName(value) {
	 ...
	 const handleSaturdayChange = async(index, value) => {
    ...
    fetch('https://lawyerback.trikara.com/api/lawyer-set-day-status?day=sat&val=' + valueToSet, {
      method: "POST",
      headers: {
...
// Handle start time change (saturday)
  const handleStartHourChangeSaturday = async(index, value) => {
  	
  	if(getValueFromName(value) == "0"){
  		 var newVal = getValueFromName(value);
    	setStartHourSat(newVal);
    	...
    	fetch('https://lawyerback.trikara.com/api/lawyer-set-start-time-sat?start_time=' + newVal, requestOptions)
      	.then((response) => response.json())
      	.then((json) => returnStartTimeSaturday(json.lawyer))
      	.catch((error) => console.error('ss'))
      	.finally(() => setLoading(false));
  	}else{
  		
    if((parseInt(getValueFromName(value))>parseInt(endHourSat))){
  ...
  const returnStartTimeSaturday = (lawyer) => {
    setStartHourSat((lawyer.start_time_hour_saturday).toString());
  }
  
  
  // Handle end time change (saturday)
  const handleEndHourChangeSaturday = async(index, value) => {
  	
  	if(getValueFromName(value) == "0"){
  		var newVal = getValueFromName(value);
    	setEndHourSat(newVal);
   	...
   	var authToken = await AsyncStorage.getItem('@auth_token');
    	...
    	fetch('https://lawyerback.trikara.com/api/lawyer-set-end-time-sat?end_time=' + newVal, requestOptions)
     		.then((response) => response.json())
      	.then((json) => returnEndTimeSaturday(json.lawyer))
      	.catch((error) => console.error('ss'))
      	.finally(() => setLoading(false));
  	}else{

    if((parseInt(getValueFromName(value))<parseInt(startHourSat))){
  ...
  const returnEndTimeSaturday = (lawyer) => {
    setEndHourSat((lawyer.end_time_hour_saturday).toString());
  }
  
  
// Handle break time start of day (saturday)    
  const handleBreakStartHourChangeSaturday = async(index, value) => {
  	
if(getValueFromName(value) == "0"){
	var newVal = getValueFromName(value);
    	setBreakStartHourSat(newVal);
    	...
    	fetch('https://lawyerback.trikara.com/api/lawyer-set-break-start-time-sat?start_time=' + newVal, requestOptions)
      	.then((response) => response.json())
      	.then((json) => returnStartTime1Saturday(json.lawyer))
      	.catch((error) => console.error('ss'))
      	.finally(() => setLoading(false));
}else{


    if((parseInt(getValueFromName(value))<parseInt(startHourSat)) || (parseInt(getValueFromName(value))>parseInt(endHourSat))){
       //message.error('Break time has to be within working hours');
       Toast.show({
                type: 'error',
                text1: 'Break time has to be within working hours',
            });
    }else if((parseInt(getValueFromName(value))>parseInt(breakEndHourSat)) && (getValueFromName(value)!=="0")&& (breakEndHourSat!=='0')){
  ...
  const returnStartTime1Saturday = (lawyer) => {
    setBreakStartHourSat((lawyer.break_start_hour_saturday).toString());
  }
  
  
  // Handle break end time of day (saturday)
  const handleBreakEndHourChangeSaturday = async(index, value) => {
    //(event) {
    ...
    if((parseInt(getValueFromName(value))<parseInt(startHourSat)) || (parseInt(getValueFromName(value))>parseInt(endHourSat))){
       ...
       Toast.show({
    ...
    }else if((parseInt(getValueFromName(value))<parseInt(breakStartHourSat)) && (getValueFromName(value)!=="0")){
    	...
    	fetch('https://lawyerback.trikara.com/api/lawyer-set-break-end-time-sat?end_time=' + newVal, requestOptions)
      	.then((response) => response.json())
      	.then((json) => returnEndTime1Saturday(json.lawyer))
      	.catch((error) => console.error('ss'))
      	.finally(() => setLoading(false));
   }
}
  ...
  const returnEndTime1Saturday = (lawyer) => {
    ...
    return (
        <View style={{ marginTop: 20 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                {props.isSaturday == 'yes' && 
                <ModalDropdown
                    options={['Holiday', 'Working']}
                    defaultValue='Working'
                    onSelect={handleSaturdayChange}
            ...
            <View style={{ paddingTop: 10 }}>
                ...
                <View style={{ flexDirection: 'row', paddingTop: 10, justifyContent: 'space-between' }}>
                    <ModalDropdown
                        ...
                        defaultValue={getFullHourName(props.startHourSat)}
                        onSelect={handleStartHourChangeSaturday}
                        style={{ borderColor: 'black', borderWidth: 1, height: 30, width: '40%', borderRadius: 5, padding: 5 }}
                    ...
                    <ModalDropdown
                        options={['06:00 AM', '07:00 AM', '08:00 AM', '09:00 AM', '10:00 AM', '11:00 AM', '12:00 AM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM', '05:00 PM', '06:00 PM', '07:00 PM', '08:00 PM', '09:00 PM', '10:00 PM']}
                        defaultValue={getFullHourName(props.endHourSat)}
                        onSelect={handleEndHourChangeSaturday}
                        style={{ borderColor: 'black', borderWidth: 1, height: 30, width: '40%', borderRadius: 5, padding: 5 }}
                        dropdownStyle={{ width: '40%', borderRadius: 5, }}
                        textStyle={styles.dropdownTitle}
                        dropdownTextStyle={styles.dropdownTitle}
            ...
            <View style={{ paddingTop: 12 }}>
                ...
                <View style={{ flexDirection: 'row', paddingTop: 10, justifyContent: 'space-between' }}>
                    <ModalDropdown
                        options={['06:00 AM', '07:00 AM', '08:00 AM', '09:00 AM', '10:00 AM', '11:00 AM', '12:00 AM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM', '05:00 PM', '06:00 PM', '07:00 PM', '08:00 PM', '09:00 PM', '10:00 PM']}
                        defaultValue={getFullHourName(props.breakStartHourSat)}
                        style={{ borderColor: 'black', borderWidth: 1, height: 30, width: '40%', borderRadius: 5, padding: 5 }}
                        dropdownStyle={{ width: '40%', borderRadius: 5, }}
                        onSelect={handleBreakStartHourChangeSaturday}
                        textStyle={styles.dropdownTitle}
                        dropdownTextStyle={styles.dropdownTitle}
                    />
                    <Text style={styles.dayText}>To</Text>
                    <ModalDropdown
                        options={['06:00 AM', '07:00 AM', '08:00 AM', '09:00 AM', '10:00 AM', '11:00 AM', '12:00 AM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM', '05:00 PM', '06:00 PM', '07:00 PM', '08:00 PM', '09:00 PM', '10:00 PM']}
                        defaultValue={getFullHourName(props.breakEndHourSat)}
                        onSelect={handleBreakEndHourChangeSaturday}
                        style={{ borderColor: 'black', borderWidth: 1, height: 30, width: '40%', borderRadius: 5, padding: 5 }}
                        dropdownStyle={{ width: '40%', borderRadius: 5, }}
                        textStyle={styles.dropdownTitle}
                        dropdownTextStyle={styles.dropdownTitle}
